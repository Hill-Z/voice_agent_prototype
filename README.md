...
- 新增独立菜单「计费中心」，包含账户、套餐、消费、流水、提醒和下载。账户查看余额与额度到期明细；套餐可查看分页购买记录、按批次续费和加购并发；消费查看逐通明细及机器人/月度统计；流水保留每笔额度到账和扣费凭证。当前原型不提供自助充值。费用按通话时长按秒计费、向上取整到分；每分钟单价由语音识别、大模型和音色共同决定，发布时冻结快照，之后改配置不影响历史通话。
...
- 当前计费原型用“3 月购 6 路、4 月加购 1 路”演示：页首并列显示余额与当前可用并发，额度和并发都按购买批次分别记到期日；消费和流水共用页首时间筛选，逐通明细、机器人统计、月度统计和导出不再各设一套时间。
- 当前原型不展示自助充值或自动充值入口；低余额时引导联系客户经理。套餐购买记录为单一分页列表，每笔独立列出金额、并发和到期日，历史额度到账继续保留供对账。
...
- 计费中心支持**余额提醒与导出留痕**：余额低于预警值、余额已用尽、已经欠费、额度即将到期四档提醒；每条提醒留记录。每次导出额度到账、扣费、通话明细、消费统计或提醒记录也留操作记录，便于对账。
